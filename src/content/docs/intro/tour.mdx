...
As an example, the following program computes people who where born in Dresden and are still alive:
```nemo
hasDied(?person) :- dateOfDeath(?person, ?date) .
result(?person) :- bornIn(?person, Dresden), ~hasDied(?person) .
```

Nemo also allows us to write this as a single rule:
```nemo
result(?person) :- bornIn(?person, Dresden), ~dateOfDeath(?person, ?date) .
```
Note that here the variable `?date` is implicitly assumed to be quantified below the negation (i.e., we are looking for a person such that for every possible date, the person did not die on that date, as opposed to finding persons for which there is a date they did not die on).
...
```nemo
total_ages(#sum(?age, ?id)) :- person(?id, _, ?age).
count(#count(?id)) :- person(?id, _, _).

avg(DOUBLE(?sum) / ?count) :- total_ages(?sum), count(?count).
